...
namespace VBControls
{
    ...
    public partial class frmVariableSpecification : Form
    {
        ...
        private void SetData(DataTable dt)
        {
            ...
            for (int i = 1; i < lstFieldList.Count; i++)
            {
                ListItem li = new ListItem(lstFieldList[i], i.ToString());
                lbIndVariables.Items.Add(li);
                //if categorical, move it to categorical box
                bool boolHascat = _dtFull.Columns[li.ToString()].ExtendedProperties.ContainsKey(VBCommon.Globals.CATEGORICAL);
                var varDistinctVals = (from row in _dtFull.Select() select row[li.ToString()]).Distinct();
                if (varDistinctVals.Count() <= 2 && Information.IsNumeric(varDistinctVals.First().ToString()))
                {
                    _dtFull.Columns[li.ToString()].ExtendedProperties[VBCommon.Globals.CATEGORICAL] = true;
                    object objBtn = button3;
                    int intNdx = lbIndVariables.FindString(li.ToString());
                    lbIndVariables.SelectedItem = lbIndVariables.Items[intNdx];
                    btnRemoveInputVariable_Click_1(objBtn, new EventArgs());
                }
                ...
                else if (boolHascat)
                {
                    if (boolHascat == true)
                    {
                        bool boolIscat = (bool)_dtFull.Columns[li.ToString()].ExtendedProperties[VBCommon.Globals.CATEGORICAL];
                        if (boolIscat)
                        {
                            object objBtn = button3;
                            int intNdx = lbIndVariables.FindString(li.ToString());
        ...
        {
            bool boolBtn4 = sender.Equals((object)button4);
            List<ListItem> items = new List<ListItem>();

            if (boolBtn4)
            {
                int intSelectedIndices = listBox2.SelectedIndices.Count;
                for (int i = 0; i < intSelectedIndices; i++)
                {
                    ListItem li = (ListItem)listBox2.Items[listBox2.SelectedIndices[i]];
                    items.Add(li);
                    _dtFull.Columns[li.ToString()].ExtendedProperties[VBCommon.Globals.CATEGORICAL] = false;
                }
        ...
        {
            bool boolBtn3 = sender.Equals((object)button3);
            List<ListItem> items = new List<ListItem>();
