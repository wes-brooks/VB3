...
namespace VBTools
{
    public class Polynomial
    {
        ...
        public void InitPolyInfoTable()
        {
            _polyDT = new DataTable("PolynomialInformationDT");
        }

        private void createResultsTable()
        {
            //throw new NotImplementedException();
            //if (_polyDT != null) _polyDT = null;
            if (_polyDT == null)
            {
                _polyDT = new DataTable("PolynomialInformationDT");
        ...
        private void computePoly(DataTable result)
        {
            ...
            {
                _polytransform[n] = _intercept + 
                    _c1 * (double)r.ItemArray[1] + 
                    _c2 * ((double)r.ItemArray[1] * (double)r.ItemArray[1]);
                //double temp = (double)r["intercept"] +
                //(double)r["coeffX"] * value +
                //(double)r["coeffXX"] * (value * value);
                n++;
            }
        }
