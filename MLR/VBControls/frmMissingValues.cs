...
namespace VBControls
{
    ...
    public partial class frmMissingValues : Form
    {
        ...
        public frmMissingValues(DataGridView dgv, DataTable dt)
        {
            ...
            _dtRI = dtRowInformation.getdtRI(dt, true);
            _dtCI = dtColumnInformation.getdtCI(dt, true);
        ...
        private bool doAction(int ndx)
        {
            ...
            else if (rbActionDelRow.Checked)
            {
                if (target == "Only This Row")
                {
                    ...
                    //I do not understand what's happening here... no time to fix it now.  made deleterow local.  cheap fix, don't deserve the beer.
                    //_dt = _tu.filterDataTableRows(_dt);
                    _dt = DeleteRow(_dt.Rows[rndx][0].ToString(), rndx);
                    _dgv.DataSource = _dt;
                    breakloop = true;
                }
                else if (target == "Entire Column")
                {
                    ...
                    //I do not understand what's happening here... no time to fix it now.  made filterrows local.  cheap fix, don't deserve the beer.
                    //_dt = _tu.filterDataTableRows(_dt);
                    _dt = filterDataTableRows(_dt);
                    _dt.AcceptChanges();
                    _dgv.DataSource = _dt;
                    breakloop = true; 
                }
        ...
        private DataTable DeleteRow(string id, int rndx)
        {
            DataTable dtcopy = _dt.Copy();

            if (!_dtRI.DTRowInfo[id])
            {
                    dtcopy.Rows[rndx].Delete();
