    ...
    public class Utilities
    {
        ...
        public class TableUtils
        {
            ...
            public DataTable filterDataTableRows(DataTable dt)
            {
                ...
                DataRow[] dr = null;
                //foreach (KeyValuePair<string,bool> kv in rstatus)
                foreach (KeyValuePair<string,bool> kv in _dtRI.DTRowInfo)
                {
                    if (!kv.Value)
                    {
                        dr = dtCopy.Select("id = '" + kv.Key + "'");
                        if (dr != null && dr.Length > 0)
                        {
                            dtCopy.Rows.Remove(dr[0]);
                            //_dtRI.setRowStatus(kv.Key, true);
                        }
            ...
            public DataTable filterRVHcols(DataTable dt)
            {
                ...
                for (int c = 0; c < dt.Columns.Count; c++)
                {
                    ...
                    if (isrv == true)
                    {
                        ...
                        if (isH == true)
                        {
                            bool isVis = (bool)dt.Columns[c].ExtendedProperties[VBCommon.Globals.HIDDEN];
                            if (isVis == true)
                            {
            ...
            public DataTable filterCatVars(DataTable dt)
            {
                ...
                for (int c = 0; c < dt.Columns.Count; c++)
                {
                    ...
                    if (hascat == true)
                    {
                        bool iscat = dt.Columns[c].ExtendedProperties.ContainsKey(VBCommon.Globals.CATEGORICAL);
                        if (iscat == true)
                        {
                            if (iscat == true)
                            {
            ...
            public DataTable addCatCols(DataTable dtnew, DataTable dt)
            {
                ...
                foreach (DataColumn dc in dt.Columns)
                {
                    ...
                    if (!dtCopy.Columns.Contains(dc.Caption))
                    {
                        ...
                        //dtCopy.Columns[dc.Caption].ExtendedProperties[VBCommon.Globals.ENABLED] = false;
            ...
            public DataTable addDisabledCols(DataTable dtnew, DataTable dt)
            {
                ...
                foreach (DataColumn dc in dt.Columns)
                {
                    ...
                    if (!hasAttribute) continue;
                    bool enabled = (bool)dc.ExtendedProperties[VBCommon.Globals.ENABLED];
                    if (!enabled)
                    {
                        if (!dtCopy.Columns.Contains(dc.Caption))
                        {
                ...
                foreach (DataColumn dc in dt.Columns)
                {
                    if (dc.ExtendedProperties.ContainsKey(VBCommon.Globals.TRANSFORM))
                    {
                        if (!dtCopy.Columns.Contains(dc.Caption))
                        {
                            ...
                            //dtCopy.Columns[dc.Caption].ExtendedProperties[VBCommon.Globals.TRANSFORM] = true;
                            dtCopy = copyAllColAttributes(dc, dt, dtCopy);
                ...
                foreach (DataColumn dc in dt.Columns)
                {
                    ...
                    if (isHidden)
                    {
                        if (!dtCopy.Columns.Contains(dc.Caption))
                        {
                            ...
                            dtCopy.Columns[dc.Caption].SetOrdinal(ndx);
                            dtCopy = copyAllColAttributes(dc, dt, dtCopy);
                            //dtCopy.Columns[dc.Caption].ExtendedProperties[VBCommon.Globals.HIDDEN] = true;
                            //dtCopy.Columns[dc.Caption].ExtendedProperties[VBCommon.Globals.DEPENDENTVAR] = true;
                        }
                    }
        ...
        public class GridUtils
        {
            ...
            public void maintainGrid(DataGridView dgv, DataTable dt, int selectedColIndex, string responseVarColName)
            {
                ...
                foreach (DataColumn c in dt.Columns)
                {
                    ...
                    if (hashidden == true)
                    {
                        bool hide = (bool)c.ExtendedProperties[VBCommon.Globals.HIDDEN];
                    ...
                    if (hasattribute)
                    {
                        ...
                        bool enabled = (bool)c.ExtendedProperties[VBCommon.Globals.ENABLED];
                        if (!enabled) 
                        {
                ...
                //    bool hasattribute = c.ExtendedProperties.ContainsKey(VBCommon.Globals.HIDDEN);
                ...
                //    }
                //    //reset the column disabled properties
                //    hasattribute = c.ExtendedProperties.ContainsKey(VBCommon.Globals.ENABLED);
                //    if (hasattribute)
                //    {
                //        selectedColIndex = dt.Columns.IndexOf(c);
                //        bool enabled = (bool)c.ExtendedProperties[VBCommon.Globals.ENABLED];
            ...
            public void unHideHiddenCols(DataGridView dgv, DataTable dt)
            {
                foreach (DataColumn dc in dt.Columns)
                {
                    ...
                    if (hasH)
                    {
                        ...
                        if (isHidden)
                        {
                            //if (!dt.Columns[dc.Caption].ExtendedProperties.ContainsKey(VBCommon.Globals.DEPENDENTVAR))
                            //{
