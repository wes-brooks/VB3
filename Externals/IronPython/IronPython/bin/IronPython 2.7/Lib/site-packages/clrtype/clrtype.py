    ...
    def emit_fields(self, typebld):
        if hasattr(self, "_clrfields"):
            for fldname in self._clrfields:
                ...
                field_builder = typebld.DefineField(
                    fldname, 
                    clr.GetClrType(field_type), 
                    FieldAttributes.Public)
                    
                #Here is where we apply custom attributes to fields
                if hasattr(self, "_field_attributes"):
                    if fldname in self._field_attributes:
                        for attr in self._field_attributes[fldname]:
                            custom_attribute = make_cab(attr)
                            field_builder.SetCustomAttribute(custom_attribute)
